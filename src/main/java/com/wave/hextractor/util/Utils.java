...
public class Utils {
	...
	/**
	 * Get a localized message from the resource bundle with parameters.
	 *
	 * @param key the message key
	...
	/**
	 * Copy the file using modern Java Files API.
	 *
	...
	/**
	 * Creates an ascii file with the content using modern Java Files API.
	 *
	 * @param file the file
	...
	/**
	 * Pads the string from the left up to size chars with '0' (Java 21 optimized).
	 *
	...
	public static String fillLeft(String text, int size) {
		...
		return padLength > 0 ? String.valueOf(Constants.PAD_CHAR_STRING).repeat(padLength) + text : text;
	}
				...
				if(Constants.HEXCHARS.contains(charString)) {
					...
					case Constants.ADDR_CHAR:
						...
						while(charGot == Constants.ADDR_CHAR) {
							...
							log(Utils.getMessage("consoleDumping", hexString));
	...
	/**
	 * Gets the text area with fixed width (adds newlines).
	 *
	 ...
	 * @return the text area string with newlines
	 */
	public static String getTextAreaFixedWidth(int offset, int length, byte[] data, HexTable hexTable, int cols) {
		StringBuilder sb = new StringBuilder(length);
		int end = offset + length;
		if(end > data.length) {
			end = data.length;
	 ...
	 * @param length the length
	 * @param data the data.
	 * @param cols the number of columns, 0 for no newlines
	 * @return the hex area
	 */
	public static String getHexArea(int offset, int length, byte[] data, int cols) {
		...
		for(int i = offset; i < end; i++) {
			...
			if(cols > 0 && curCol == cols - 1 && i < end - 1) {
				sb.append(Constants.S_NEWLINE);
				curCol = 0;
			} else {
	...
	public static String getHexArea(int offset, int length, byte[] data) {
		return getHexArea(offset, length, data, 0);
	}
	...
	public static String getHexAreaFixedWidth(int offset, int length, byte[] data, int cols) {
		return getHexArea(offset, length, data, cols);
	}
	...
	/**
	 * Logs an exception by printing its stack trace.
	 *
	...
	/**
	 * Returns the directory where the application is running from (the JAR location
	 * or the classes folder during development). Falls back to the current working
	 * directory if it cannot be determined.
	 *
	 * @return the jar directory as File
	 */
	public static File getJarDirectory() {
		try {
			java.net.URI uri = com.wave.hextractor.Hextractor.class
					.getProtectionDomain()
					.getCodeSource()
					.getLocation()
					.toURI();
