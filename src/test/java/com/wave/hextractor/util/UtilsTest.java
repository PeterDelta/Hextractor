...
public class UtilsTest {
	...
	public void getHexFilledLeft() {
		assertEquals("0000", Utils.getHexFilledLeft(0, 4));
	}

	@Test
	public void toHexString() {
		byte[] data = {0, 1, 2, 3, 4};
		assertEquals("[00 01 02 03 04 ]", Utils.toHexString(data));
	}

	@Test
	public void hexStringToByteArray() {
		byte[] data = {0, 1, 2, 3, 4};
		assertArrayEquals(data, Utils.hexStringToByteArray("0001020304"));
	}

	@Test
	public void hexStringCharToByte() {
		assertEquals((byte) 0, Utils.hexStringCharToByte("00"));
	}
	...
	public void getOffsets() throws IOException {
		...
		assertEquals(resEntries, Utils.getOffsets(file.getAbsolutePath()));
		assertEquals(resEntries, Utils.getOffsets(offsets));
	}
	...
	public void getCleanedString() {
		assertEquals("abcd x'yz", Utils.getCleanedString("´Abcd    X'YZ"));
	}
	...
	public void allSameValue() {
		byte[] eq = {1, 1, 1, 1};
		byte[] neq = {2, 1, 4, 1};
		assertTrue(Utils.allSameValue(eq));
		assertFalse(Utils.allSameValue(neq));
	}

	@Test
	public void intToHexString() {
		assertEquals("0001", Utils.intToHexString(1, 4));
	}
	...
	public void hexStringListToIntList() {
		...
		int[] intValues = {1, 2, 3};
		assertArrayEquals(intValues, Utils.hexStringListToIntList(hexValues));
	}
	...
	public void getHexOffsets() {
		...
		assertEquals(offEntries, Utils.getHexOffsets(entries));
	}
	...
	public void sortByValue() {
		...
		assertEquals(valuesSorted, valuesInit);
	}
	...
	public void toFileString() {
		...
		assertEquals("00000000-00000001-FF-00,00000005-0000000A-01-00", Utils.toFileString(offEntries));
	}
	...
	public void extractDictionary() {
		...
		assertEquals(resMap, Utils.extractDictionary(lines));
	}

	@Test
	public void checkLineLength() {
		assertTrue(Utils.checkLineLength("text1#105", "texto1#105"));
		assertFalse(Utils.checkLineLength("text1#105", "texto1#106"));
	}
	...
	public void getTextArea() {
		...
		assertEquals("ABCD", Utils.getTextArea(1, 4, bytes, table));
	}

	@Test
	public void getHexArea() {
		byte[] bytes = {0x40, 0x41, 0x42, 0x43, 0x44, 0x45};
		assertEquals("41 42 43 44 ", Utils.getHexArea(1, 4, bytes));
	}
	...
	public void removeCommentsAndJoin() {
		...
		assertArrayEquals(res, Utils.removeCommentsAndJoin(file));
	}
	...
	public void getCompressed4To3Data() {
		...
		assertArrayEquals(bytesComp, Utils.getCompressed4To3Data(bytes));

		byte[] bytes2 = {0, 1, 2, 3, 4, 5};
		byte[] bytesComp2 = {0, 16, -125, 16};
		assertArrayEquals(bytesComp2, Utils.getCompressed4To3Data(bytes2));

		byte[] bytes3 = {0, 1, 2, 3, 4, 5, 6};
		byte[] bytesComp3 = {0, 16, -125, 16, 81};
		assertArrayEquals(bytesComp3, Utils.getCompressed4To3Data(bytes3));
	}
	...
	public void getExpanded3To4Data() {
		...
		assertArrayEquals(bytesDecomp, Utils.getExpanded3To4Data(bytesComp));

		byte[] bytesComp2 = {0, 16, -125, 16};
		byte[] bytesDecomp2 = {0, 1, 2, 3, 4, 0};
		assertArrayEquals(bytesDecomp2, Utils.getExpanded3To4Data(bytesComp2));

		byte[] bytesComp3 = {0, 16, -125, 16, 81};
		byte[] bytesDecomp3 = {0, 1, 2, 3, 4, 5, 4};
		assertArrayEquals(bytesDecomp3, Utils.getExpanded3To4Data(bytesComp3));
	}
	...
	public void bytesToHex() {
		byte[] bytes = {1, 2, 3, 4};
		assertEquals("01020304", Utils.bytesToHex(bytes));
	}

	@Test
	public void testParseMode() {
		assertEquals(Hextractor.Mode.EXTRACT_ASCII, Hextractor.parseMode("-a"));
		assertEquals(Hextractor.Mode.HEX_VIEW, Hextractor.parseMode("-hv"));
		assertNull(Hextractor.parseMode("invalid"));
	}
}
